...
# arson attack to a player without cards
pc = PlayersControl()
gc = GameControl(EventList(), test_data.CardPool(test_data.gen_cards([
            test_data.CardInfo('arson attack', 1, card.DIAMOND),
...
# arson attack | 1    | DIAMOND <- use this
# arson attack | 2    | DIAMOND <- use this
# arson attack | 3    | HEART <- use this
# arson attack | 4    | HEART <- use this
# slash        | 9    | SPADE
# slash        | 10   | CLUB
...
# arson attack to a player itself, with only the arson attack card
pc = PlayersControl()
gc = GameControl(EventList(), test_data.CardPool(test_data.gen_cards([
            test_data.CardInfo('arson attack', 1, card.DIAMOND),
            test_data.CardInfo('arson attack', 2, card.DIAMOND),
...
# arson attack | 1    | DIAMOND <- use this
# arson attack | 2    | DIAMOND
# arson attack | 3    | HEART
# arson attack | 4    | HEART
# arson attack | 9    | HEART
# arson attack | 10   | HEART
...
last_event_id = len(gc.get_events(players[0].token, 0)) # until arson attack
...
# arson attack | 3    | HEART <- show this
# arson attack | 4    | HEART
# arson attack | 9    | HEART
# arson attack | 10   | HEART
...
if True: # just indent for a nice appearance
    ...
    assert_eq(players[0].player_id, event['player'])
    assert_eq(1, len(event['show']))
    assert_eq('arson attack', event['show'][0]['name'])
...
# arson attack | 3    | HEART
# arson attack | 4    | HEART <- discard this
# arson attack | 9    | HEART
# arson attack | 10   | HEART
...
if True: # just indent for a nice appearance
    ...
    assert_eq(players[0].player_id, event['player'])
    assert_eq(1, len(event['discard']))
    assert_eq('arson attack', event['discard'][0]['name'])
...
# arson attack | 3    | HEART <- show this
# arson attack | 10   | HEART <- use this
...
# arson attack | 3    | HEART <- use this, no cards after use it
