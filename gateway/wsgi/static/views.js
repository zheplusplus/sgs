...
function Game(pane) {
    ...
    function paintCard(coord, card, position) {
        ctxt.save();
    ...
    function Me(ctxt, coord) {
        ...
        function clickOnCard(c) {
            ...
            if (index != -1) {
                ...
                paintCard(coord.offset(LEFT_AREA, 0), card, index);
                return true;
            }
            return false;
        };
        ...
        function repaintCards() {
            ...
            for (i = 0; i < cards.length; ++i) {
                paintCard(coord.offset(LEFT_AREA, 0), cards[i], i);
            }
        }
        function resetRight() {
        ...
        this.selectCharacter = function(name, max_vigor) {
            ...
            ctxt.fillText(Array(max_vigor + 1).join('[]'), 0, TEXT_H,
                          LEFT_AREA);
            ctxt.restore();
        };
        this.useCards = function() {
    ...
    function Center(ctxt, coord) {
        ...
        this.selectCharacters = function(candidates) {
            ctxt.save();
            ctxt.translate(coord.x, coord.y);
            var width = (CENTER_W - (CHILD_HORI_INTERVAL * (candidates.length -
                            1))) / candidates.length;
            ctxt.textBaseline = 'top';
